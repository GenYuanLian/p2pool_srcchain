...
from util import fixargparse, jsonrpc, variable, deferral, math, logging, switchprotocol, pack
...
class keypool():
    ...
    def paytotal(self):
        ...
        for i in range(len(pubkeys.keys)):
            self.payouttotal += node.get_current_txouts().get(bitcoin_data.pubkey_hash_to_script2(pubkeys.keys[i]),
                                                              0) * 1e-8
    ...
    try:
        ...
        def poll_warnings():
            bitcoind_getinfo_var.set(
                (yield deferral.retry('Error while calling getinfo:')(bitcoind.rpc_getnetworkinfo)()))
        ...
        if args.pubkey_hash is None and args.address != 'dynamic':
            ...
            for i in range(args.numaddresses):
                address = yield deferral.retry('Error getting a dynamic address from bitcoind:', 5)(
        ...
        wb = work.WorkerBridge(node, my_pubkey_hash, args.donation_percentage, merged_urls, args.worker_fee, args,
                               pubkeys, bitcoind)
        ...
        serverfactory = switchprotocol.FirstByteSwitchFactory({'{': stratum.StratumServerFactory(caching_wb)},
                                                              web_serverfactory)
        deferral.retry('Error binding to worker port:', traceback=False)(reactor.listenTCP)(worker_endpoint[1],
                                                                                            serverfactory,
                                                                                            interface=worker_endpoint[
                                                                                                0])
        ...
        global_var.set_value('script', pubkey_hash_to_script2(
            address_to_pubkey_hash(global_var.get_value('reserve_address'), wb.net)))
                ...
                def signedOn(self):
                    ...
                    def new_share(share):
                        if not self.in_channel:
                            return
                        if share.pow_hash <= share.header['bits'].target and abs(
                                        share.timestamp - time.time()) < 10 * 60:
                            yield deferral.sleep(random.expovariate(1 / 60))
                            message = '\x02%s BLOCK FOUND by %s! %s%064x' % (
                                net.NAME.upper(), bitcoin_data.script2_to_address(share.new_script, net.PARENT),
                                net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
                            if all('%x' % (share.header_hash,) not in old_message for old_message in
            ...
            while True:
                ...
                try:
                    ...
                    this_str = 'P2Pool: %i shares in chain (%i verified/%i total) Peers: %i (%i incoming)' % (
                        ...
                        len(reactor.getReaders()), len(reactor.getWriters())) if p2pool.DEBUG else '')

                    datums, dt = wb.local_rate_monitor.get_datums_in_last()
                    my_att_s = sum(datum['work'] / dt for datum in datums)
                    my_shares_per_s = sum(
                        datum['work'] / dt / bitcoin_data.target_to_average_attempts(datum['share_target']) for datum in
                        datums)
                    ...
                    if height > 2:
                        ...
                        stale_prop = p2pool_data.get_average_stale_prop(node.tracker, node.best_share_var.value,
                                                                        min(60 * 60 // net.SHARE_PERIOD, height))
                        real_att_s = p2pool_data.get_pool_attempts_per_second(node.tracker, node.best_share_var.value,
                                                                              min(height - 1,
                        ...
                        paytot = 0.0
                        for i in range(len(pubkeys.keys)):
                            curtot = node.get_current_txouts().get(bitcoin_data.pubkey_hash_to_script2(pubkeys.keys[i]),
                                                                   0)
                            paytot += curtot * 1e-8
                        ...
                        for warning in p2pool_data.get_warnings(node.tracker, node.best_share_var.value, net,
...
def run():
    ...
    parser = fixargparse.FixedArgumentParser(description='p2pool (version %s)' % (p2pool.__version__,),
    ...
    bitcoind_group.add_argument('--srcchaind-rpc-port', metavar='BITCOIND_RPC_PORT',
                                help='''connect to JSON-RPC interface at this port (default: %s <read from srcchain.conf if password not provided>)''' % ', '.join(
    ...
    bitcoind_group.add_argument('--srcchaind-p2p-port', metavar='BITCOIND_P2P_PORT',
                                help='''connect to P2P interface at this port (default: %s <read from srcchain.conf if password not provided>)''' % ', '.join(
    ...
    if not args.testnet:
        global_var.set_value('donation',
                             pubkey_hash_to_script2(974143861276139069219601755531477429285430505615))  # livenet
    else:
        global_var.set_value('donation',
                             pubkey_hash_to_script2(519791236046945930428491375523868623218951526243))  # testnet
    p2pool_data.BaseShare.gentx_before_refhash = pack.VarStrType().pack(
        global_var.get_value('donation')) + pack.IntType(64).pack(
        0) + pack.VarStrType().pack('\x6a\x28' + pack.IntType(256).pack(0) + pack.IntType(64).pack(0))[:3]
